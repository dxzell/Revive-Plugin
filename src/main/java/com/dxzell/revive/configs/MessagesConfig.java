...
public class MessagesConfig extends Config {
    ...
    public String getPlayerDiedMessage(String playerName) {
        List<String> messages = config.getStringList("messages.player-died")
                .stream()
                .map(message -> message.replace("[player]", playerName))
                .collect(Collectors.toList());

        return (messages != null && !messages.isEmpty() ? messages.get(getRandomNum(messages.size())) : "");
    }

    public String getPlayerKnockedMessage(String playerName) {
        List<String> messages = config.getStringList("messages.player-knocked")
                .stream()
                .map(message -> message.replace("[player]", playerName))
                .collect(Collectors.toList());
